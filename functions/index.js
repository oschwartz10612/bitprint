...
exports.coinbaseWebhook = functions.https.onRequest(
  async (request, response) => {
    ...
    if (type == "confirmed") {
      admin.firestore().doc(`orders/${user.paymentEndpoint.code}`).set({
        order: user.cart,
        uid: user.uid,
        timestamp: getDate(),
      ...
      admin.firestore().doc(`users/${user.uid}`).update({
        order: user.cart,
        cart: [],
        totalCost: 0,
        paymentStatus: type,
...
exports.stripeWebhook = functions.https.onRequest(async (request, response) => {
  ...
  if (event.type == "checkout.session.completed") {
    ...
    admin.firestore().doc(`orders/${event.data.object.payment_intent}`).set({
      order: user.cart,
      uid: user.uid,
      timestamp: getDate(),
      shippingAddress: user.shippingAddress,
